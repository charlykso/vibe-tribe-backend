...
class EnvironmentManager {
  constructor() {
    ...
    this.environments = {
      development: {
        ...
        JWT_SECRET: 'demo-super-secret-jwt-key-for-development-only-change-in-production',
      ...
      staging: {
        ...
        TWITTER_REDIRECT_URI: 'https://staging-vibe-tribe-backend.onrender.com/api/v1/oauth/twitter/callback',
        LINKEDIN_REDIRECT_URI: 'https://staging-vibe-tribe-backend.onrender.com/api/v1/oauth/linkedin/callback',
        FACEBOOK_REDIRECT_URI: 'https://staging-vibe-tribe-backend.onrender.com/api/v1/oauth/facebook/callback',
        INSTAGRAM_REDIRECT_URI: 'https://staging-vibe-tribe-backend.onrender.com/api/v1/oauth/instagram/callback',
      ...
      for (const line of lines) {
        ...
        if (trimmed && !trimmed.startsWith('#')) {
          ...
          if (key && valueParts.length > 0) {
            existing[key.trim()] = valueParts.join('=').trim().replace(/^["']|["']$/g, '');
          }
        }
      }
    }
  ...
  mergeEnvironments(targetEnv, existing) {
    ...
    for (const [key, value] of Object.entries(envConfig)) {
      if (value === null) {
        ...
        if (!merged[key]) {
          console.warn(`⚠️  Warning: ${key} is not set and is required for ${targetEnv}`);
        }
      } else {
  ...
  generateSecurityKeys(config) {
    ...
    if (!config.TOKEN_ENCRYPTION_KEY || config.TOKEN_ENCRYPTION_KEY.includes('dev')) {
  ...
  validateEnvironment(config, targetEnv) {
    ...
    const required = [
      'NODE_ENV', 'PORT', 'JWT_SECRET', 'TOKEN_ENCRYPTION_KEY',
    ...
    if (targetEnv === 'production') {
      ...
      for (const field of sensitiveFields) {
        if (config[field] && (config[field].includes('demo') || config[field].includes('dev'))) {
    ...
    for (const platform of oauthPlatforms) {
      ...
      const clientSecret = config[`${platform}_CLIENT_SECRET`] || config[`${platform}_APP_SECRET`];
  ...
  writeEnvFile(config) {
    ...
    const groups = {
      'Server Configuration': ['NODE_ENV', 'PORT', 'FRONTEND_URL', 'CORS_ORIGIN'],
      'Security Configuration': ['JWT_SECRET', 'JWT_EXPIRES_IN', 'TOKEN_ENCRYPTION_KEY', 'BCRYPT_ROUNDS'],
      ...
      'Twitter OAuth': ['TWITTER_CLIENT_ID', 'TWITTER_CLIENT_SECRET', 'TWITTER_REDIRECT_URI'],
      ...
      'External Services': ['CLOUDINARY_CLOUD_NAME', 'CLOUDINARY_API_KEY', 'CLOUDINARY_API_SECRET', 'OPENAI_API_KEY'],
    ...
    for (const [groupName, keys] of Object.entries(groups)) {
      ...
      for (const key of keys) {
        if (config[key] !== undefined) {
          ...
          const quotedValue = value && (value.includes(' ') || value.includes('\n')) 
            ? `"${value}"` 
            : value;
  ...
  async run(targetEnv = 'auto') {
    ...
    const environment = targetEnv === 'auto' ? this.detectEnvironment() : targetEnv;
    
    if (!this.environments[environment]) {
      console.error(`❌ Unknown environment: ${environment}`);
      console.log('Available environments:', Object.keys(this.environments).join(', '));
      process.exit(1);
    }
    ...
    if (environment === 'development' || !config.JWT_SECRET || !config.TOKEN_ENCRYPTION_KEY) {
