...
export const keyGenerators = {
  ...
  byIP: (req: AuthenticatedRequest) => `ip:${req.ip}`,
  
  // Rate limit by user ID
  byUser: (req: AuthenticatedRequest) => 
    req.user ? `user:${req.user.id}` : `ip:${req.ip}`,
  
  // Rate limit by organization
  byOrganization: (req: AuthenticatedRequest) => 
    req.user?.organization_id ? `org:${req.user.organization_id}` : `ip:${req.ip}`,
  
  // Rate limit by user and endpoint
  byUserAndEndpoint: (req: AuthenticatedRequest) => 
    req.user ? `user:${req.user.id}:${req.route?.path || req.path}` : `ip:${req.ip}`,
  
  // Rate limit by API key (if using API keys)
  byAPIKey: (req: AuthenticatedRequest) => {
