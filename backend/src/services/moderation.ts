...
export class ModerationService {
  ...
  async addToModerationQueue(queueData: Partial<ModerationQueue> & { organization_id: string; content_type: string; content_id: string; reason: string }): Promise<ModerationQueue> {
    try {
      ...
      return { id: doc.id, ...doc.data() } as ModerationQueue;
    } catch (error) {
  ...
  async moderateContent(
    ...
    try {
      ...
      return { id: actionDoc.id, ...actionDoc.data() } as ModerationAction;
    } catch (error) {
  ...
  private async getModerationQueueItem(queueItemId: string): Promise<ModerationQueue | null> {
    const db = getFirestoreClient();
  ...
  async createModerationRule(ruleData: Partial<ModerationRule> & { organization_id: string; name: string; rule_type: string; conditions: Record<string, any>; actions: Record<string, any> }): Promise<ModerationRule> {
    try {
      const db = getFirestoreClient();
      ...
      const doc = await docRef.get();
      return { id: doc.id, ...doc.data() } as ModerationRule;
    } catch (error) {
  ...
  async getModerationRules(organizationId: string, communityId?: string): Promise<ModerationRule[]> {
    try {
      const db = getFirestoreClient();
      let query = db
  ...
  async processMessage(message: CommunityMessage): Promise<void> {
    try {
      ...
      const rules = await this.getModerationRules(
        message.community_id // Note: This should be organization_id, but we need to get it from community
      );
  ...
  private async checkRule(message: CommunityMessage, rule: ModerationRule): Promise<any> {
    try {
  ...
  private checkKeywordRule(content: string, conditions: any): any {
  ...
  private checkRegexRule(content: string, conditions: any): any {
  ...
  private async checkSentimentRule(message: CommunityMessage, conditions: any): Promise<any> {
    if (message.sentiment_score === undefined) return null;
  ...
  private async checkToxicityRule(message: CommunityMessage, conditions: any): Promise<any> {
    if (message.toxicity_score === undefined) return null;
  ...
  private async checkSpamRule(message: CommunityMessage, conditions: any): Promise<any> {
    if (message.spam_score === undefined) return null;
  ...
  private async runAIAnalysis(message: CommunityMessage): Promise<void> {
    try {
      // Run sentiment analysis
      const sentimentAnalysis = await aiService.analyzeSentiment(
        message.content,
        '', // Should get organization_id from community
        message.id
      );

      // Run toxicity analysis
      const toxicityAnalysis = await aiService.analyzeToxicity(
        message.content,
        '', // Should get organization_id from community
        message.id
      );
      ...
      // Run spam detection
      const spamAnalysis = await aiService.detectSpam(
        message.content,
        '', // Should get organization_id from community
        message.id
      );
  ...
  private async executeContentAction(queueItem: ModerationQueue, action: string): Promise<void> {
    try {
  ...
  private async executeAutoAction(message: CommunityMessage, action: string): Promise<void> {
    try {
      const db = getFirestoreClient();
      switch (action) {
        case 'delete':
  ...
  async getModerationStats(organizationId: string, days = 30): Promise<any> {
    try {
  ...
  private getTopReasons(queueItems: any[]): any[] {
  ...
  private getDailyStats(queueItems: any[], days: number): any[] {
