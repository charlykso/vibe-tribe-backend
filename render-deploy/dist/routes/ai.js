    ...
    try {
        ...
        for (const analysisType of analysisTypes) {
            try {
                switch (analysisType) {
                    case 'sentiment':
                        results.sentiment = await aiService.analyzeSentiment(validatedData.content, organization_id, validatedData.content_id);
                        break;
                    case 'toxicity':
                        results.toxicity = await aiService.analyzeToxicity(validatedData.content, organization_id, validatedData.content_id);
                        break;
                    case 'spam':
                        results.spam = await aiService.detectSpam(validatedData.content, organization_id, validatedData.content_id);
                        break;
                }
            }
            catch (error) {
...
router.get('/content-suggestions', authMiddleware, async (req, res) => {
    try {
        ...
        for (const prompt of prompts) {
            try {
                const content = await aiService.generateContent(platform ? `${prompt} for ${platform}` : prompt, req.user.organization_id);
                suggestions.push({
                    prompt,
                    content,
...
router.get('/analytics', authMiddleware, async (req, res) => {
    try {
        ...
        const analytics = {
            ...
            daily_usage: Array.from({ length: parseInt(days) }, (_, i) => ({
