...
router.post('/invite', asyncHandler(async (req, res) => {
    ...
    try {
        await emailService.sendInvitationEmail(email, user.name, organization.name, invitationToken);
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const invitationsQuery = await firestore
        ...
        .get();
    const invitations = await Promise.all(invitationsQuery.docs.map(async (doc) => {
...
router.post('/:invitationId/resend', asyncHandler(async (req, res) => {
    ...
    try {
        await emailService.sendInvitationEmail(invitation.email, user.name, organization.name, invitation.invitation_token);
