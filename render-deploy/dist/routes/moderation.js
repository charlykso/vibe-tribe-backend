...
router.get('/queue', authMiddleware, async (req, res) => {
    try {
        ...
        const queueItems = await moderationService.getModerationQueue(organization_id, status, parseInt(limit), parseInt(offset));
        res.json({
            success: true,
            data: queueItems
        });
...
router.post('/queue/:id/moderate', authMiddleware, async (req, res) => {
    try {
        ...
        const moderationAction = await moderationService.moderateContent(id, validatedData.action, moderatorId, validatedData.notes);
        res.json({
            success: true,
...
router.get('/rules', authMiddleware, async (req, res) => {
    try {
        ...
        const rules = await moderationService.getModerationRules(organization_id, community_id);
        res.json({
            success: true,
            data: rules
        });
...
router.get('/stats', authMiddleware, async (req, res) => {
    try {
        ...
        const stats = await moderationService.getModerationStats(organization_id, parseInt(days));
        res.json({
            success: true,
            data: stats
        });
...
router.post('/rule-templates/:id/apply', authMiddleware, async (req, res) => {
    try {
