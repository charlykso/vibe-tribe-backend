...
async function getOAuthState(state) {
    try {
        ...
        const key = getRedisKey(REDIS_PREFIX.OAUTH_STATE, state);
        const stateData = await getAndParse(key);
        if (!stateData) {
...
router.post('/initiate', authMiddleware, ...oauthSecurityMiddleware.initiate, asyncHandler(async (req, res) => {
...
router.post('/callback', authMiddleware, ...oauthSecurityMiddleware.callback, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (accountData.refresh_token) {
            ...
            await TokenRefreshScheduler.getInstance().scheduleTokenRefresh(accountId, platform, expiresAt);
        }
...
router.post('/refresh', authMiddleware, ...oauthSecurityMiddleware.refresh, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const oauthService = OAuthServiceFactory.getService(platform);
        const refreshResult = await oauthService.refreshAccessToken(accountData.refresh_token);
        if (!refreshResult) {
        ...
        await TokenRefreshScheduler.getInstance().scheduleTokenRefresh(accountId, platform, updatedData.expiresAt);
...
router.get('/status/:platform', authMiddleware, asyncHandler(async (req, res) => {
...
router.delete('/disconnect/:accountId', authMiddleware, asyncHandler(async (req, res) => {
...
router.get('/twitter/callback', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!existingAccountQuery.empty) {
            ...
            accountData = { id: existingDoc.id, ...updateData };
            ...
            accountData = { id: docRef.id, ...newAccountData };
...
router.get('/debug/twitter-url', async (req, res) => {
    try {
        ...
        const result = await twitterService.generateAuthUrl(testState);
...
router.get('/debug/linkedin-url', async (req, res) => {
    try {
        ...
        const authUrl = linkedinService.generateAuthUrl(testState);
        // Store state in Redis for testing (same as real OAuth flow)
        await storeOAuthState(testState, {
            userId: 'debug_user_123',
...
router.get('/debug/facebook-url', async (req, res) => {
    try {
        ...
        const authUrl = facebookService.generateAuthUrl(testState);
...
router.get('/debug/instagram-url', async (req, res) => {
    try {
        ...
        const authUrl = instagramService.generateAuthUrl(testState);
        // Store state in Redis for testing
        await storeOAuthState(testState, {
            userId: 'debug_user_123',
            organizationId: '51eehsR75718t2svEqhh',
