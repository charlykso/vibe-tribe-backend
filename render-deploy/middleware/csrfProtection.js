...
export function csrfProtection(options = {}) {
    const { ignoreMethods = ['GET', 'HEAD', 'OPTIONS'], headerName = 'x-csrf-token', bodyField = '_csrf' } = options;
    return (req, res, next) => {
        try {
    ...
    return (req, res, next) => {
        try {
            ...
            csrfProtection()(req, res, (error) => {
                ...
                if (maxAge) {
                    const tokenTimestamp = req.headers['x-csrf-timestamp'];
                    if (tokenTimestamp) {
