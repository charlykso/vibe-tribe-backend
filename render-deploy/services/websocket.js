...
export const initializeWebSocket = (io) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('subscribe:posts', (data) => {
        ...
        socket.on('unsubscribe:posts', (data) => {
            if (data.post_ids && Array.isArray(data.post_ids)) {
                data.post_ids.forEach(postId => {
...
export const emitPostUpdate = (io, postId, data) => {
...
export const emitAnalyticsUpdate = (io, organizationId, data) => {
    io.to(`analytics:${organizationId}`).emit('analytics:updated', {
        ...data,
        timestamp: new Date().toISOString()
    });
};
export const emitSocialAccountUpdate = (io, organizationId, data) => {
