...
function generateOAuthBase64() {
  ...
  const missingKeys = Object.keys(oauthCredentials).filter(key => !oauthCredentials[key]);
...
function generateFirebaseBase64() {
  ...
  if (missingFields.length > 0) {
    ...
    missingFields.forEach(field => console.log(`   - FIREBASE_${field.toUpperCase()}`));
    console.log('\n   Please set these in your .env file first.\n');
    return null;
  }
...
function main() {
  try {
    ...
    console.log('3. The app will automatically use Base64 if individual vars are missing');
    console.log('4. Individual variables take priority over Base64 when both are present\n');
