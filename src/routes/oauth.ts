...
router.post('/initiate', asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    // Store state information in Redis (expires in 10 minutes)
    const stateData = {
      userId: user.id,
    ...
    try {
      await redis.set(getStateKey(state), JSON.stringify(stateData), 'EX', 600);
    } catch (err) {
      console.error('❌ Redis error setting OAuth state:', err);
      throw new ValidationError('Failed to store OAuth state');
    }
...
router.post('/callback', asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  let stateDataRaw: string | null = null;
  try {
    stateDataRaw = await redis.get(getStateKey(state));
  } catch (err) {
    console.error('❌ Redis error getting OAuth state:', err);
    throw new UnauthorizedError('Failed to verify OAuth state');
  }
  if (!stateDataRaw) {
  ...
  // Clean up the state in Redis
  try {
    await redis.del(getStateKey(state));
  } catch (err) {
    console.error('❌ Redis error deleting OAuth state:', err);
  }
...
// TEMP: Redis health check endpoint
router.get('/redis-health', asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ status: 'error', error: err instanceof Error ? err.message : err });
  }
}));
