...
router.get('/overview', asyncHandler(async (req, res) => {
    ...
    const socialAccounts = socialAccountsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    const topPlatform = Object.entries(platformEngagement).reduce((top, [platform, engagement]) => engagement > (platformEngagement[top] || 0) ? platform : top, 'LinkedIn');
...
router.get('/engagement', asyncHandler(async (req, res) => {
    ...
    if (platform) {
        platformData = {
            platform,
            total_engagement: engagementData.reduce((sum, day) => sum + day.likes + day.comments + day.shares, 0),
            avg_engagement_rate: (engagementData.reduce((sum, day) => sum + day.engagement_rate, 0) / days).toFixed(2),
            best_performing_day: engagementData.reduce((best, current) => (current.likes + current.comments + current.shares) >
                (best.likes + best.comments + best.shares) ? current : best, engagementData[0])
        };
    }
    res.json({
...
router.get('/top-posts', asyncHandler(async (req, res) => {
    const firestore = getFirestoreClient();
