...
class FacebookPublisher {
    ...
    async publishToUserFeed(post, account) {
        try {
            const postPayload = {
...
export const publishToSocialPlatforms = async (post) => {
    try {
        ...
        const relevantAccounts = accounts.filter(account => post.platforms.includes(account.platform));
...
export const getConnectedAccounts = async (organizationId) => {
    const firestore = getFirestoreClient();
...
export const validatePostForPlatforms = (post) => {
    const errors = [];
